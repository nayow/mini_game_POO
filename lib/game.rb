class Game
    ...
    #adds new players randomly at each turn
    ...
    #keeps playing if user and at least one enemy are still alive 
    def is_still_ongoing?
        human_player.life_points > 0 && players_left >= 1 ? true : false
    end

    #show both the user's and bots' status
    def show_players
        human_player.show_state
        puts "Il reste #{players_left} ennemi(s)"
    end

    #either search weapon or health or kill an enemy in sight
    def menu
    ...
    #enemies in sight counterattack
    def enemies_attack
